...
def main() -> None:
    ...
    fees = parse_billings_mbox(args.mbox)
    if not fees:
    ...
    if os.path.abspath(args.out) == os.path.abspath(args.orders):
        stamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
        os.makedirs(args.backup_dir, exist_ok=True)
        base = os.path.basename(args.orders)
        backup_path = os.path.join(args.backup_dir, f"{base}.{stamp}.bak")
        shutil.copy2(args.orders, backup_path)
        print(f"Backed up original CSV -> {backup_path}")
    merged.to_csv(args.out, index=False)
    print(f"Updated {len(merged)} rows with proc/comm fees -> {args.out}")
    ...
    if not missing_rows.empty:
        ...
        missing_rows.to_csv(args.missing_out, index=False)
        print(
            f"Missing proc/comm fees for {len(missing_rows)} order_id(s). "
            f"Wrote list -> {args.missing_out}"
        )
